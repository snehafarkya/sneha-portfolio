...
const Playlist = () => {
  ...
  const videos = [
    ...
    },
    {
      ...
      title:
        "How I made money by freelancing | Tips and Tricks | Answering your queries",
      date: "12 July 2023",
      image: freelance,
    },
    {
        ...
        <div className="flex md:flex-row flex-col md:gap-24 md:justify-between justify-center">
          ...
          <div className="video-list">
            {videos.map((video, index) => (
              <div
                key={index}
                className="video-item flex md:flex-row flex-col mx-6  gap-4  md:w-[470px] my-6 cursor-pointer rounded-lg text-gray-800  bg-white order-2 expcard"
                onClick={() => handleVideoClick(index)}
              >
                <img
                  src={video.image}
                  className="md:w-56 z-10 md:rounded-tl-lg md:rounded-tr-none  md:rounded-bl-lg rounded-t-lg"
                />
                <div className="flex flex-col px-4">
                  <p className="text-ellipsis line-clamp-2 my-4 text-base font-bold">
                    {video.title}
                  </p>

                  <p className="text-xs mb-10 md:mb-0 font-medium">
                    Date: {video.date}
                  </p>
                </div>
              </div>
